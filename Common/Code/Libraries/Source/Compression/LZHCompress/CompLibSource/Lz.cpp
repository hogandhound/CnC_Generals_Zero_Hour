...
size_t LZHLCompressor::compress( BYTE* dst, const BYTE* src, size_t sz )
    ...
    for(;;)
        ...
        intptr_t nRaw = 0;
        intptr_t maxRaw = min( srcLeft - LZMATCH, LZHLEncoder::maxRaw );
        #ifdef LZLAZYMATCH
        int    lazyMatchLen = 0;
        ...
        for(;;)
            ...
            if( hashPos != (LZTABLEINT)(-1) && hashPos != wrapBufPos )
                {
                intptr_t matchLimit = min( min( _distance( wrapBufPos - hashPos ), srcLeft - nRaw ), LZMIN + LZHLEncoder::maxMatchOver );
                matchLen = _nMatch( hashPos, src + nRaw, (int)matchLimit );

                #ifdef LZOVERLAP
                if( _wrap( hashPos + matchLen ) == wrapBufPos )
                    {
                    assert( matchLen != 0 );
                    intptr_t xtraMatchLimit = min( LZMIN + LZHLEncoder::maxMatchOver - matchLen, srcLeft - nRaw - matchLen );
                ...
                if( matchLen >= LZMIN - 1 )//to ensure that buf will be overwritten
                    {
                    intptr_t xtraMatchLimit = min( LZMIN + LZHLEncoder::maxMatchOver - matchLen, nRaw );
            ...
            if( lazyMatchLen >= LZMIN )
                ...
                if( matchLen > lazyMatchLen )
                    ...
                    hash = _updateTable( hash, src + nRaw, bufPos + 1, (int)min( matchLen - 1, srcEnd - (src + nRaw + 1) ) );
                    ...
                    hash = _updateTable( hash, src + nRaw + 1, bufPos + 2, (int)min( lazyMatchLen - 2, srcEnd - (src + nRaw + 2) ) );
            ...
            if( nRaw + 1 > maxRaw )
                ...
                if( lazyMatchLen >= LZMIN )
                    {
                    coder.putMatch( src, nRaw, lazyMatchLen - LZMIN, _distance( bufPos - lazyMatchHashPos ) );
                    hash = _updateTable( hash, src + nRaw, bufPos + 1, (int)min( lazyMatchLen - 1, srcEnd - (src + nRaw + 1) ) );
                    _toBuf( src + nRaw, lazyMatchLen );
                    src += nRaw + lazyMatchLen;
                    break;//for( nRaw )
                    }
