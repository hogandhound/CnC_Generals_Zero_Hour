...
static int _getInt( BYTE*& p, int sz, size_t* val )
...
int lzhl_send( SOCKET sock, const char* data, int dataSz, int flags )
{
	...
	if( iter != globalMap.end() )
	{
		...
		{
		BYTE* p = buf + 10 - dSz;
		int sz = (int)(dSz + compSz);
		while( sz )
		{
			int bytes = send( sock, (char*)( buf + 10 - dSz ), (int)(dSz + compSz), flags );
			if( bytes < 0 )
				return bytes;
	...
	if( iter != globalMap.end() )
	{
		...
		if( ls.dBuf == 0 )
		{
			...
			for(;;)
			{
				...
				err = _getInt( p, (int)(bytesRead - ( p - tmp )), &compSz );
				if( err == 1 )
					continue;//forever
				else if( err > 0 )
					return -1;
			...
			for(;;)
			{
				int bytes = recv(sock, (char*)(compBuf + bytesRead), (int)(compSz - bytesRead), flags);
				if( bytes <= 0 )
				{
					delete [] compBuf;
					return bytes;
