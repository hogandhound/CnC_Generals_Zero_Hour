...
layout(location = 2) in vec2 fragUv2; //Modified Cam
layout(location = 3) in vec2 fragUv3; //Modified Cam 2

layout(location = 0) out vec4 finalColor;

void main() {
    vec4 r0 = fragDiffuse * texture(tex1, fragUv1);
	r0.rgb = (texture(tex2, fragUv1) * texture(tex3, fragUv2) + r).rgb * texture(tex4, fragUv3).rgb;
	finalColor = r0;
}
