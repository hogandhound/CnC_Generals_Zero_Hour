...
void main() {
	vec4 normal = texture(texNorm,fragUv);
	normal.xyz = 2.0 * (normal.xyz - 0.5) * fragSpec.xyz;
	vec4 decal = texture(texDecal,fragUv);
	decal.rgb = fragDiff.xyz * decal.rgb;
	
	vec4 r0 = clamp(decal * normal, 0.0, 1.0);
	vec4 r1 = clamp(decal * -normal, 0.0, 1.0);
	r0 = r0 + r1;
	r0 = decal * fragSpec.a + r0;
	r1 = clamp(decal.a * normal,0,1);
    finalColor = fragSpecUv * r1 + r0;
}
