...
layout(location = 3) out vec4 fragSpecUv;
	...
	light.x = clamp(dot(halfAngle, worldNorm),0,1) * bump.lightColor[0].w;
	light.x = light.x * light.x * light.x * light.x; //light.x ^ 4?
	col = bump.lightColor[0] * light.x;
	fragSpecUv =  col*bump.specular;
#endif
...
void main() {
	...
	fragDiff.w = bump.constVal.y - light.x;
	
	#if 0 //I don't think this does anything
	vec4 world = push.world * vec4(vert,0.0);
	eye.xyz = normalize(bump.eyeWorld - world.xyz);
	eye.w = 1.0;
	
	vec4 halfAngle = bump.lightDir[0] + eye;
	halfAngle.xyz = normalize(halfAngle.xyz);
	halfAngle.w = 1.0;
	
	lightLocal.x = dot(wS.xyz, halfAngle.xyz);
	lightLocal.y = dot(wT.xyz, halfAngle.xyz);
	lightLocal.z = dot(wSxT.xyz, halfAngle.xyz);
	lightLocal.xyz = normalize(lightLocal.xyz);
	lightLocal.w = 1.0;
	lightLocal.xyz = lightLocal.xyz * bump.bumpiness.zzz;
	
	light.x = clamp(dot(halfAngle.xyz * worldNorm.xyz),0,1) * bump.lightColor[0].w;
	light.x = light.x * light.x * light.x * light.x;
	#endif
}
