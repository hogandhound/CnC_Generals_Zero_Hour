...
void PartitionCell::removeLooker(Int playerIndex)
{
	...
	{
		DEBUG_ASSERTLOG( m_shroudLevel[playerIndex].m_currentShroud < 0, ("Someone is RemoveLooker-ing on a cell that is not looked at.  This will make a permanent shroud blob.") );
		m_shroudLevel[playerIndex].m_currentShroud++;
	}
	CellShroudStatus newShroud = getShroudStatusForPlayer( playerIndex );
...
void PartitionData::allocCoiArray()
{
	...
	m_coiArray = new CellAndObjectIntersection[m_coiArrayCount];	// may throw!
	m_coiInUseCount = 0;
	makeDirty(true);
}
...
void PartitionManager::init()
{
	...
	if (TheTerrainLogic)
	{
		...
		m_cells = new PartitionCell[m_totalCellCount];
...
)
{
	...
	DEBUG_ASSERTLOG(theEntrancyCount == 0, ("sorry, this routine is not reentrant"));
	++theEntrancyCount;
#endif
...
void PartitionManager::undoShroudCover(Real centerX, Real centerY, Real radius, PlayerMaskType playerMask) 
{
	...
	for( Int currentIndex = ThePlayerList->getPlayerCount() - 1; currentIndex >=0; currentIndex-- )
	{
		...
		if( BitTestWW( playerMask, currentPlayer->getPlayerMask() ) )
		{
			circle.drawCircle(hLineRemoveShrouder, (void*)(uintptr_t)currentIndex);
		}
	}
}
...
void PartitionManager::xfer( Xfer *xfer )
{
	...
	if( version >= 2 )
	{
		Int queueSize = (int)m_pendingUndoShroudReveals.size();
...
//		return false;
	...
	DEBUG_ASSERTLOG(m_obj && m_obj->isAbleToAttack(), ("if the object is unable to attack at all, you should filter that out ahead of time!"));
#endif
...
Bool PartitionFilterStealthedAndUndetected::allow( Object *objOther )
{
	...
	if( BitTestWW( objOther->getStatusBits(), OBJECT_STATUS_STEALTHED ) && !BitTestWW( objOther->getStatusBits(), OBJECT_STATUS_DETECTED ) )
	{
	...
	else
	{
		...
		if( contain )
		{
			...
			if( contain->getStealthUnitsContained() == contain->getContainCount() && contain->getContainCount() > 0 )
			{
				...
				Object *member = (*it);
				if( member && !BitTestWW( (*it)->getStatusBits(), OBJECT_STATUS_DETECTED ) )
				{
