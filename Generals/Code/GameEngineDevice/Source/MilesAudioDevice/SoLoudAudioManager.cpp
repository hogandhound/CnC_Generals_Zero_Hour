...
class WWSoLoudFile : public SoLoud::File
{
private:
	::File* file;
public:
	WWSoLoudFile(::File* file) : file(file) {};
	~WWSoLoudFile() { file->close(); }
	...
	{
		return file->read((void*)aDst, (int)aBytes);
	}
...
//-------------------------------------------------------------------------------------------------
SoLoudAudioManager::SoLoudAudioManager() :
	m_providerCount(0),
...
AudioHandle SoLoudAudioManager::addAudioEvent(const AudioEventRTS* eventToAdd)
{
...
void SoLoudAudioManager::audioDebugDisplay(DebugDisplayInterface* dd, void*, FILE* fp)
{
	...
	if (dd)
	{
		...
		dd->printf("%d(%d) 3D Sounds\n", m_sound->getAvailable3DSamples(), m_available3DSamples);
		dd->printf("Volume: ");
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::postProcessLoad()
{
	AudioManager::postProcessLoad();
}

//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::reset()
{
#if defined(_DEBUG) || defined(_INTERNAL)
	dumpAllAssetsUsed();
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::update()
{
	AudioManager::update();
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::stopAudio(AudioAffect which)
{
	...
	if (BitTestWW(which, AudioAffect_Sound)) {
		for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
			playing = *it;
			if (playing) {
				this->m_digitalHandle.stop(playing->m_handle);
...
void SoLoudAudioManager::pauseAudio(AudioAffect which)
{
	...
	if (BitTestWW(which, AudioAffect_Sound)) {
		for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
			playing = *it;
			if (playing) {
				this->m_digitalHandle.setPause(playing->m_handle, true);
			}
		}
	}
...
void SoLoudAudioManager::resumeAudio(AudioAffect which)
{
	...
	if (BitTestWW(which, AudioAffect_Sound)) {
		for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
			playing = *it;
			if (playing) {
				m_digitalHandle.setPause(playing->m_handle,0);
			}
		}
	}
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::pauseAmbient(Bool shouldPause)
{

}

//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::playAudioEvent(AudioEventRTS* event)
{
	...
	case AT_Streaming:
	{
		...
		SoLoud::Wav* stream = nullptr;
		if (!handleToKill || foundSoundToReplace) {
			::File* file = TheFileSystem->openFile(fileToPlay.str(), File::READ | File::STREAMING);
			if (file) {
				stream = new SoLoud::Wav();
				stream->loadFile(new WWSoLoudFile(file));
			}
			//stream = m_digitalHandle.play(wav);
	...
	case AT_SoundEffect:
	{
		...
		if (event->isPositionalAudio()) {
			...
			if (sample3D) {
				audio->m_handle = playSample3D(event, audio->m_file);
		...
		else
		{
			...
			if (sample) {
				audio->m_handle = playSample(event, audio->m_file);
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::stopAudioEvent(AudioHandle handle)
{
#ifdef INTENSIVE_AUDIO_DEBUG
	...
	for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
		...
		if (audio->m_audioEventRTS->getPlayingHandle() == handle) {
			...
			notifyOfAudioCompletion(audio->m_handle, PAT_Stream);
			break;
		}
	}
...
void SoLoudAudioManager::stopAllAmbientsBy(Object* objID)
{
}

void SoLoudAudioManager::stopAllAmbientsBy(Drawable* drawID)
{
}


//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::pauseAudioEvent(AudioHandle handle)
{
	// pause audio
}

//-------------------------------------------------------------------------------------------------
std::shared_ptr<SoLoud::Wav> SoLoudAudioManager::loadFileForRead(AudioEventRTS* eventToLoadFrom)
{
	return m_audioCache->openFile(eventToLoadFrom);
}

//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::closeFile(std::shared_ptr<SoLoud::Wav>& fileRead)
{
...
//-------------------------------------------------------------------------------------------------
PlayingAudio* SoLoudAudioManager::allocatePlayingAudio(void)
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::releaseMilesHandles(PlayingAudio* release)
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::releasePlayingAudio(PlayingAudio* release)
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::freeAllMilesHandles(void)
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::adjustPlayingVolume(PlayingAudio* audio)
{
	...
	if (audio->m_type == PAT_Sample) {
		...
		if (audio->m_audioEventRTS->getAudioEventInfo()->m_soundType == AT_Music) {
			m_digitalHandle.setVolume(audio->m_handle, m_musicVolume * desiredVolume);

		}
		else {
			m_digitalHandle.setVolume(audio->m_handle, m_speechVolume * desiredVolume);
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::initFilters(SoLoud::handle sample, const AudioEventRTS* event)
{
	// set the sample volume
	Real volume = event->getVolume() * event->getVolumeShift() * m_soundVolume;
	m_digitalHandle.setVolume(sample, volume);
	...
	if (pitchShift == 0.0f) {
		...
		m_digitalHandle.setRelativePlaySpeed(sample,pitchShift);
	...
	if (event->getDelay() > 0.0f) {
		...
		m_digitalHandle.setDelaySamples(sample, value * m_digitalHandle.getSamplerate(sample));
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::initFilters3D(SoLoud::handle sample, const AudioEventRTS* event, const Coord3D* pos)
{
	// set the sample volume
	...
	if (event->getAudioEventInfo()->m_lowPassFreq > 0 && !isOnScreen(pos)) {
		SoLoud::BiquadResonantFilter* filter = nullptr;
		if (!filter)
		{
			filter = new SoLoud::BiquadResonantFilter();
			//filter->setParams(SoLoud::BiquadResonantFilter::LOWPASS, event->getAudioEventInfo()->m_lowPassFreq, 1.0);
			m_digitalHandle.setGlobalFilter(1, filter);
		}
		m_digitalHandle.setFilterParameter(sample, 1, SoLoud::BiquadResonantFilter::LOWPASS, event->getAudioEventInfo()->m_lowPassFreq);
		//auto inst = filter.createInstance();
		//inst->initParams(1);
		//inst->setFilterParameter(SoLoud::BiquadResonantFilter::LOWPASS, );
		//m_digitalHandle.setFilterParameter(sample, 0, SoLoud::Filter::, 1.0f - event->getAudioEventInfo()->m_lowPassFreq);
	...
	for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
		...
		if (playing && playing->m_audioEventRTS->getAudioEventInfo()->m_soundType == AT_Music) {
			if (playing->m_audioEventRTS->getEventName() == trackName) {
				...
				if (INFINITE_LOOP_COUNT - ((SoLoud::Soloud&)m_digitalHandle).getLoopCount(playing->m_handle) >= (uint32_t)numberOfTimes) {
					return TRUE;
				}
			}
		}
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::openDevice(void)
{
	if (!TheGlobalData->m_audioOn) {
		return;
	}

	m_digitalHandle.init();
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::closeDevice(void)
{
	freeAllMilesHandles();
	unselectProvider();
	//AIL_shutdown();
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::notifyOfAudioCompletion(SoLoud::handle audioCompleted, UnsignedInt flags)
{
	...
	if (playing->m_audioEventRTS->getNextPlayPortion() != PP_Done) {
		if (playing->m_type == PAT_Sample) {
			...
			playing->m_handle = playSample(playing->m_audioEventRTS, playing->m_file);
			...
			closeFile(playing->m_file);	// close it so as not to leak it.
			playing->m_handle = playSample3D(playing->m_audioEventRTS, playing->m_file);
	...
	if (playing->m_type == PAT_Stream) {
		if (playing->m_audioEventRTS->getAudioEventInfo()->m_soundType == AT_Music) {
			playing->m_handle = playStream(playing->m_audioEventRTS, *playing->m_stream);
...
PlayingAudio* SoLoudAudioManager::findPlayingAudioFrom(SoLoud::handle audioCompleted, UnsignedInt flags)
{
	...
	if (flags == PAT_Sample) {
		SoLoud::handle sample = audioCompleted;
		for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
			playing = *it;
			if (playing && playing->m_handle == sample) {
				return playing;
			}
		}
	}
	...
	if (flags == PAT_3DSample) {
		SoLoud::handle sample3D = audioCompleted;
		for (it = m_playing3DSounds.begin(); it != m_playing3DSounds.end(); ++it) {
			playing = *it;
			if (playing && playing->m_handle == sample3D) {
				return playing;
			}
		}
	}
	...
	if (flags == PAT_Stream) {
		SoLoud::handle stream = audioCompleted;
		for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
			playing = *it;
			if (playing && playing->m_handle == stream) {
				return playing;
			}
		}
	}
...
//-------------------------------------------------------------------------------------------------
UnsignedInt SoLoudAudioManager::getProviderCount(void) const
{
	return m_providerCount;
}

//-------------------------------------------------------------------------------------------------
AsciiString SoLoudAudioManager::getProviderName(UnsignedInt providerNum) const
{
...
//-------------------------------------------------------------------------------------------------
UnsignedInt SoLoudAudioManager::getProviderIndex(AsciiString providerName) const
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::selectProvider(UnsignedInt providerNdx)
{
	...
	//int channels = m_digitalHandle.getBackendChannels();
	Bool useDolby = FALSE;
#if 0
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::unselectProvider(void)
{
...
//-------------------------------------------------------------------------------------------------
UnsignedInt SoLoudAudioManager::getSelectedProvider(void) const
{
	return m_selectedProvider;
}

//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::setSpeakerType(UnsignedInt speakerType)
{
	if (!isValidProvider()) {
		return;
	}
...
//-------------------------------------------------------------------------------------------------
UnsignedInt SoLoudAudioManager::getSpeakerType(void)
{
	if (!isValidProvider()) {
		return 0;
	}
...
//-------------------------------------------------------------------------------------------------
UnsignedInt SoLoudAudioManager::getNum2DSamples(void) const
{
	return m_num2DSamples;
}

//-------------------------------------------------------------------------------------------------
UnsignedInt SoLoudAudioManager::getNum3DSamples(void) const
{
	return m_num3DSamples;
}

//-------------------------------------------------------------------------------------------------
UnsignedInt SoLoudAudioManager::getNumStreams(void) const
{
	return m_numStreams;
}

//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::doesViolateLimit(AudioEventRTS* event) const
{
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::isObjectPlayingVoice(UnsignedInt objID) const
{
	if (objID == 0) {
		return false;
	}
...
//-------------------------------------------------------------------------------------------------
AudioEventRTS* SoLoudAudioManager::findLowestPrioritySound(AudioEventRTS* event)
{
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::isPlayingLowerPriority(AudioEventRTS* event) const
{
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::killLowestPrioritySoundImmediately(AudioEventRTS* event)
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::processPlayingList(void)
{
			...
			if (pos)
			{
				...
				else
				{
					...
					else
					{
						...
						Real y = pos->y;
						Real z = pos->z;
						m_digitalHandle.set3dSourcePosition(playing->m_handle, x, y, z);
...
Bool SoLoudAudioManager::has3DSensitiveStreamsPlaying(void) const
{
...
void SoLoudAudioManager::processFadingList(void)
{
	...
	for (it = m_fadingAudio.begin(); it != m_fadingAudio.end(); /* emtpy */) {
		...
		case PAT_Sample:
		{
			m_digitalHandle.setVolume(playing->m_handle, volume);
			break;
		}
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::shouldProcessRequestThisFrame(AudioRequest* req) const
{
	if (!req->m_usePendingEvent) {
		return true;
	}
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::adjustRequest(AudioRequest* req)
{
	if (!req->m_usePendingEvent) {
		return;
	}
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::checkForSample(AudioRequest* req)
{
	if (!req->m_usePendingEvent) {
		return true;
	}
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::setHardwareAccelerated(Bool accel)
{
	// Extends
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::setSpeakerSurround(Bool surround)
{
	// Extends
...
//-------------------------------------------------------------------------------------------------
Real SoLoudAudioManager::getFileLengthMS(AsciiString strToLoad) const
{
	...
	SoLoud::Wav stream;
	stream.load(strToLoad.str());
	SoLoud::time time = stream.getLength();
...
void SoLoudAudioManager::closeAnySamplesUsingFile(const void* vfileToClose)
{
	...
	for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ) {
		...
		if (playing->m_file.get() == fileToClose) {
			releasePlayingAudio(playing);
			it = m_playingSounds.erase(it);
		}
		else {
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::setDeviceListenerPosition(void)
{
	m_digitalHandle.set3dListenerPosition(m_listenerPosition.x, m_listenerPosition.y, m_listenerPosition.z);
	m_digitalHandle.set3dListenerUp(0, 0, -1);
	//if (m_listener) {
...
//-------------------------------------------------------------------------------------------------
const Coord3D* SoLoudAudioManager::getCurrentPositionFromEvent(AudioEventRTS* event)
{
	if (!event->isPositionalAudio()) {
		return NULL;
	}
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::isOnScreen(const Coord3D* pos) const
{
...
//-------------------------------------------------------------------------------------------------
Real SoLoudAudioManager::getEffectiveVolume(AudioEventRTS* event) const
{
	Real volume = 1.0f;
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::startNextLoop(PlayingAudio* looping)
{
	...
	if (looping->m_audioEventRTS->hasMoreLoops()) {
		...
		if (looping->m_type == PAT_3DSample) {
			looping->m_handle = playSample3D(looping->m_audioEventRTS, looping->m_file);
		}
		else {
			looping->m_handle = playSample(looping->m_audioEventRTS, looping->m_file);
		}
...
SoLoud::handle SoLoudAudioManager::playStream(AudioEventRTS* event, SoLoud::Wav& stream)
{
	...
	if (event->getAudioEventInfo()->m_soundType == AT_Music) {
		...
		m_digitalHandle.setLooping(handle, true);
	}
...
//-------------------------------------------------------------------------------------------------
SoLoud::handle SoLoudAudioManager::playSample(AudioEventRTS* event, std::shared_ptr<SoLoud::Wav>& sample)
{
...
//-------------------------------------------------------------------------------------------------
SoLoud::handle SoLoudAudioManager::playSample3D(AudioEventRTS* event, std::shared_ptr<SoLoud::Wav>& sample3D)
{
	...
	if (pos) {
		...
		if (sample3D) {
			...
			if (event->getAudioEventInfo()->m_type & ST_GLOBAL) {
				...
				m_digitalHandle.set3dSourceMinMaxDistance(handle, event->getAudioEventInfo()->m_minDistance, event->getAudioEventInfo()->m_maxDistance);
			}
			initFilters3D(handle, event, pos);
#if 0
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::buildProviderList(void)
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::createListener(void)
{
	if (!(isOn(AudioAffect_Sound3D) && isValidProvider())) {
		return;
	}
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::initDelayFilter(void)
{
...
//-------------------------------------------------------------------------------------------------
Bool SoLoudAudioManager::isValidProvider(void)
{
	//Disable this
	return true;// (m_selectedProvider < m_providerCount);
}

//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::initSamplePools(void)
{
	if (!(isOn(AudioAffect_Sound3D) && isValidProvider())) {
		return;
	}
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::processRequest(AudioRequest* req)
{
...
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::friend_forcePlayAudioEventRTS(const AudioEventRTS* eventToPlay)
{
	...
	AsciiString fileToPlay = event.getFilename();
	SoLoud::Wav wav;
	wav.load(fileToPlay.str());
	SoLoud::handle haud = m_digitalHandle.play(wav);
	...
	m_digitalHandle.setVolume(haud, event.getVolume() * getVolume(AudioAffect_Speech));
...
uintptr_t SL_streamingFileOpen(char const* fileName, File** file_handle)
{

	(*file_handle) = TheFileSystem->openFile(fileName, File::READ | File::STREAMING);
	return ((*file_handle) != 0);
}
...
//-------------------------------------------------------------------------------------------------
std::shared_ptr<SoLoud::Wav> AudioFileCache::openFile(AudioEventRTS* eventToOpenFrom)
{
	...
	std::shared_ptr<SoLoud::Wav> wav = std::make_unique<SoLoud::Wav>();
	wav->loadFile(new WWSoLoudFile(file));
...
//-------------------------------------------------------------------------------------------------
void AudioFileCache::closeFile(std::shared_ptr<SoLoud::Wav>& fileToClose)
{
	if (!fileToClose) {
		return;
	}
...
void AudioFileCache::releaseOpenAudioFile(OpenAudioFile* fileToRelease)
{
	if (fileToRelease->m_openCount > 0) {
		...
		TheAudio->closeAnySamplesUsingFile(fileToRelease->m_file.get());
	}
...
#if defined(_DEBUG) || defined(_INTERNAL)
//-------------------------------------------------------------------------------------------------
void SoLoudAudioManager::dumpAllAssetsUsed()
{
	if (!TheGlobalData->m_preloadReport) {
		return;
	}
