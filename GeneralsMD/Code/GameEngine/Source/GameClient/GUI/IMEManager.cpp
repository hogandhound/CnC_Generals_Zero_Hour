	...
	if ( m_context )
	{
		...
		else
		{
			...
			if ( result > 0 )
			{
				...
				int convRes = MultiByteToWideChar( CP_ACP, 0, tempBuf, (int)(int)strlen(tempBuf), m_resultsString, MAX_COMPSTRINGLEN );
...
{
 	int size = MultiByteToWideChar( CP_ACP, 0, mbcs, (int)(int)strlen(mbcs), NULL, 0 );
