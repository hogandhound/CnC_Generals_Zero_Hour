...
void SurfaceClass::Unlock(VK::Texture* tex, VK::SamplerSettings samp)
{
#ifdef INFO_VULKAN
...
void SurfaceClass::Clear(VK::Texture* texture)
{
	...
	if (texture)
	{
		...
		VK::CreateTexture(&WWVKRENDER, *texture, surface.width, surface.height, surface.buffer.data(),
			texture->sampSettings, surface.format);
	}
#ifdef INFO_VULKAN
	D3DLOCKED_RECT lock_rect;	
	::ZeroMemory(&lock_rect, sizeof(D3DLOCKED_RECT));
...
void SurfaceClass::Copy(const unsigned char *other, VK::Texture* texture)
{
	...
	memcpy(surface.buffer.data(), other, surface.width * surface.height * VK::SizeOfFormat(surface.format));
	if (texture)
	{
		VK::CreateTexture(&WWVKRENDER, *texture, surface.width, surface.height, surface.buffer.data(),
			texture->sampSettings, surface.format);
