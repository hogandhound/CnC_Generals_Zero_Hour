...
void VertexMaterialClass::Parse_Mapping_Args(const W3dVertexMaterialStruct & vmat,char * mapping0_arg_buffer,char * mapping1_arg_buffer)
{
	...
	if (mapping0_arg_buffer) {
		...
		int mapping0_arg_len = (int)strlen(mapping0_arg_buffer);
		...
		mapping0_arg_len = (unsigned int)(strlen(extended_arg_buffer) + 1);
	...
	if (mapping1_arg_buffer) {
		...
		int mapping1_arg_len = (int)strlen(mapping1_arg_buffer);

		char *extended_arg_buffer = MSGW3DNEWARRAY("VertexMaterialClassTemp") char[mapping1_arg_len + 20];
		sprintf(extended_arg_buffer, "[Args]\n%s", mapping1_arg_buffer);
		mapping1_arg_len = (unsigned int)(strlen(extended_arg_buffer) + 1);
...
{
	int i;
	static DX8Material default_settings = 
	{
	...
	DX8Wrapper::Set_DX8_Render_State(VKRS_AMBIENTMATERIALSOURCE, ColorSourceType::MATERIAL);
	DX8Wrapper::Set_DX8_Render_State(VKRS_DIFFUSEMATERIALSOURCE, ColorSourceType::MATERIAL);
	DX8Wrapper::Set_DX8_Render_State(VKRS_EMISSIVEMATERIALSOURCE, ColorSourceType::MATERIAL);
